...
import 'package:flutter_braille_typing/views/braille_ui.dart';
import 'package:flutter_braille_typing/views/keyboard_focus_node.dart';
...
class MyHomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String inputLetter = Braille.brailleBoolToChar(Provider.of<InputState>(context).keys) ?? " ";
    ...
    if (Provider.of<AppOptionsState>(context).darkTheme == true) brightness=Icons.mode_night;
    if (Provider.of<AppOptionsState>(context).darkTheme == false) brightness=Icons.light_mode;
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            onPressed: () {
              Provider.of<AppOptionsState>(context, listen: false).changeTheme();
            }, icon: Icon(brightness) ,
            tooltip: Provider.of<AppOptionsState>(context, listen: false).darkTheme == null ? "Theme (using system's)" : "Theme"
          ),
        ],
      ),
      body:
             ...
             crossAxisAlignment: CrossAxisAlignment.start,
               children: [
                 ...
                 TextField(
                    ...
                    controller: Provider.of<GameState>(context, listen: false).controller,
                  ),
               ]),
          ),
          Flexible(
    ...
    for (int i=0;i< Braille.numberOfDots ;i++){
      dotData.add(
        BrailleDotData(
          ...
          letter: Provider.of<AppOptionsState>(context).showKeyboardLetters ? MyPhysicalKeyboard.letters[i] : " ",
          id: i ,
        )
      );
    }
        ...
        Flexible(
          ...
          child: Center(child: ElevatedButton(
            ...
            child: Padding(
              ...
              child: Text( (rightAnswer ? "✅ Next" : "Clear") + (Provider.of<AppOptionsState>(context).showKeyboardLetters ? " [Spacebar]" : "") ,
...
class TargetBrailleCharacter extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i=0;i<Braille.numberOfDots ;i++){
      dotData.add(
        BrailleDotData(
          state: hide ? false : (Braille.charToBrailleBoolList(target)?.value[i] ?? false ),
          letter: hide ? "?" : "", id: i
        )
      );
    }
...
showLicensePage(BuildContext context) async {
  await showDialog(context: context, builder: (context) => LicensePage(
    ...
    applicationLegalese: "${DateTime.now().year}, by ADHSoft\nAuthor: Alejandro Herme\n"
